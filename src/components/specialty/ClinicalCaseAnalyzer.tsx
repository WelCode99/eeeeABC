...
export const ClinicalCaseAnalyzer: React.FC<ClinicalCaseAnalyzerProps> = ({ specialty }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Analyze clinical data for suggestions
      const clinicalData = caseData.clinicalData;
      if (clinicalData.age && clinicalData.comorbidities) {
        const newSuggestions = [];
        
        // Check age and comorbidities for specific recommendations
        if (parseInt(clinicalData.age as string) > 65) {
          newSuggestions.push('Considerar ajuste de dose para idade avançada');
        }
        
        const comorbidities = (clinicalData.comorbidities as string).toLowerCase();
        if (comorbidities.includes('dm') || comorbidities.includes('diabetes')) {
          newSuggestions.push('Monitorar glicemia e ajustar medicações hipoglicemiantes');
        }
        
        if (comorbidities.includes('irc') || comorbidities.includes('renal')) {
          newSuggestions.push('Ajustar dosagens considerando função renal');
          newSuggestions.push('Preferir medicamentos com menor excreção renal');
        }
      ...
      {suggestions.length > 0 && (
        <div className="mt-4 p-4 bg-blue-50 rounded-lg">
          <h4 className="text-sm font-medium text-blue-900 mb-2">
            Sugestões Baseadas no Perfil do Paciente:
          </h4>
